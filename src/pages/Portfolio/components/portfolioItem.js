...
const PortfolioItem = ({ data, setFormOpen }) => {
  ...
  return (
    <div ref={itemRef} className={`${ data.cssIdentifier } cursor-pointer`} onClick={() => {
      return data.isOnRequest ?
        setFormOpen(true) :
        openDetails()
    }
    }>
      <div className="fade-up">
        ...
        <div className="mt-2 laptop:mt-6 flex flex-row justify-between">
          <div>
            <div className="font-medium text-base laptop:text-[30px]">{data.projName}</div>
            <div className="mt-2 laptop:mt-2 font-normal text-subtle text-[14px] laptop:text-[20px]">{data.projTopic}</div>
          </div>
