        ...
        <div className="  w-full p-4 bg-[#272D36] rounded-lg fade-up">
            <div className="w-full h-[40px]">
                <div className="w-full h-full flex flex-row justify-between items-center">
                    <div className={`${data.iconDim} h-full`}>
                        <img
                            src={data.iconURL}
                    ...
                    <div className="h-full flex flex-row">
                        {data.countries.map((d, index) => (
                            <Flag key={index} img={d} />
                        ))}
                    </div>
                </div>
            </div>
            <div className="w-full h-[120px] font-light text-subtle mt-5">
                {data.text}
            </div>
        </div>
    )
...
function Client() {
    ...
    useEffect(() => {
        if (isRevealed) {
            gsap.to(".client .fade-up", AnimConfig.fadeUpConfig)
        }
    }, [isRevealed])
